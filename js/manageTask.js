      ...
      tasks.forEach(task => {
        ...
        itemTask.innerHTML = `
          <div class="task-card flex flex-col gap-5">
            <div class="flex gap-3 items-center">
              ...
              <div class="flex flex-col">
                ...
                <p class="text-sm leading-[21px] text-taskia-grey">Created at ${formatDate(task.createdAt)}</p>
              </div>
            </div>
            <div class="flex gap-4 font-semibold text-sm leading-[21px]">
              ...
              <div class="flex gap-1 items-center ${task.isComplated ? "text-taskia-green" : ""}">
                <div class="flex shrink-0 w-5 h-5">
                ...
                <p>${task.isComplated ? "Completed" : "In Progress"}</p>
              </div>
            </div>
          </div>
          <div class="flex flex-row items-center gap-x-3">
            ...
            <a href="#" id="completeTask-${task.id}" class="${task.isComplated ? "hidden" : ""}  flex gap-[10px] justify-center items-center text-white p-[12px_20px] h-12 font-semibold bg-gradient-to-b from-[#977FFF] to-[#6F4FFF] rounded-full w-full border border-taskia-background-grey">Complete</a>
          </div>`;
        ...
        itemTask.querySelector(`#completeTask-${task.id}`).addEventListener('click', function (event) {
          ...
          myTasks.complateTask(task.id);
          const updateTasks = myTasks.getTask();
